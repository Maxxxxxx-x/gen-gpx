...
def train_model(feature_queue, model_state, sequence_length, batch_size, checkpoint_dir):
    ...
    try:
        ...
        if not deltas or len(deltas) < sequence_length:
            feature_queue.task_done()
            return None

        # 分割序列用於訓練
        X, y = gpx_processor.split_sequences(deltas, sequence_length)
        if not X or not y:
            feature_queue.task_done()
            return None
        ...
        model_state['trained_models'] = model_state.get(
            'trained_models', 0) + 1
        feature_queue.task_done()
